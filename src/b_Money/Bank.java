...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			accountlist.put(accountid, new Account(accountid, getCurrency()));//open account wasn't adding an account
		}
	}
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {// accountlist.containsKey(accountid) was missing a ! sign before which made it throw an exception
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			Money moni = new Money(money.getAmount() * -1, money.getCurrency());//account.deposit() was adding the value to the amount instead of subtracting it
			account.deposit(moni);
		}
	}
	...
	public void transfer(String fromaccount, Bank tobank, String toaccount, Money amount) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(fromaccount) || !tobank.accountlist.containsKey(toaccount)) { // accountlist.containsKey(fromaccount) was missing a ! sign before which made it throw an exception
			throw new AccountDoesNotExistException();
		}
		else {
